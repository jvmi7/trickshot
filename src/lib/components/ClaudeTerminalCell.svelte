<script lang="ts">
  ...
  // bar. The TUI's OWN input is the input (the custom composer experiment
  // retired everywhere but the homepage — the CLI's composer brings slash
  // menus, @-completion, and ↑-history for free). Rendered by
  // ClaudeTerminalPane once per VISIBLE chat (one in tabs layout, all of
  // them in grid), so this is the grid-ready unit: adding a cell is just
  // rendering another instance. Feature component (stores + session).
