...
| GitHub PRs (create + checks via `gh` CLI, "fix failing checks → agent") | `src-tauri/src/github.rs` (commands) + the PR block in `GitPanel.svelte` |
...
| Workspace archiving + History (archive keeps transcript/session for restore) | `stores.ts › archivedWorkspaces` + archive/restore/purge flows in `Worktrees.svelte` |
| Integrated terminal (one PTY per worktree) | `src-tauri/src/terminal.rs` (portable-pty commands) + `src/lib/terminal.ts` (xterm cache + `term-event` router) + `TerminalPane.svelte` |
