...
| The chat pane (the real Claude Code TUI; MULTIPLE concurrent chats per worktree, rendered as tabs or an n-up grid) | `ChatTabs.svelte` (the strip, on the shell band OUTSIDE the card — App.svelte mounts it) + `ClaudeTerminalPane.svelte` (the in-card surface: focused cell / grid) + `ClaudeTerminalCell.svelte` (one chat's terminal — the TUI's OWN input is the input; the custom `ChatComposer` survives ONLY on the homepage via `HomeComposer`, injecting via `sendToCli`) + `stores.ts › chatSessionsByWorktree/focusedChatByWorktree/chatLayout/chatStatusByKey` + `session.ts › ensureClaudeOpen/sendToCli/submitTurnToChat/handleCliExit` + `agent.rs › latest_session_id` |
...
| Git review (status/diff/stage/commit/push/merge) | `src-tauri/src/worktree.rs` (commands). The header ± POPOVER (`ViewToggle`, `stores.ts › changesOpen`) is the COMPACT `GitQuickPanel.svelte` — file count + ONE stateful lifecycle button (Create PR → Update PR → Merge PR → Archive worktree; `session.ts › archiveWorkspace` is the shared archive path). The FULL review surface (`GitPanel.svelte` + `DiffView.svelte`) opens as a modal from its files row (`stores.ts › reviewDialogOpen`, dialog mounted by `ViewToggle`) |
...
| Project scripts (repo `.trickshot/settings.json`: setup/run/archive, `TRICKSHOT_PORT` block) | `src-tauri/src/scripts.rs` (commands) + `src/lib/scriptEvents.ts` (router, 16ms-batched) + `RunScripts.svelte` (header Run button) + `RunWindow.svelte`/`RunOutput.svelte` (the floating run widget, per-worktree `runOpen`) |
...
- **App CSS lives in `@layer components`** (`app.css`) so Tailwind's `utilities` layer always wins for shadcn primitives. The only deliberately-unlayered element selectors are at the foot of `app.css`: (1) the `cursor` rules (`input,textarea,[contenteditable] { cursor:text }` / `button:not(:disabled),a,[role=button],… { cursor:pointer }`), which must reach `<button data-slot="button">` to set the right cursor and set only `cursor` so the bleed is wanted; (2) `[data-slot="button"]:active { translate:none; transform:none }`, which cancels the shadcn Button base's `active:translate-y-px` press-down nudge app-wide (it's an `@layer utilities` rule, so only an unlayered rule beats it); and (3) the tokenized `ui/*`-slot chrome overrides — `[data-slot="dialog-overlay"]` (the themed scrim) and `[data-slot="tooltip-content"]` (radius/shadow/ring matched to the app's floating-panel language) — the sanctioned way to restyle a generated primitive's chrome WITHOUT hand-editing its file. Any OTHER element selector belongs in `@layer components`, or it bleeds onto shadcn primitives.
