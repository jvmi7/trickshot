...
- **Auth: Claude uses the existing Claude Code login (NO key); GLM is the ONE exception.** For the `claude` provider the embedded native binary handles auth — don't add API-key plumbing there. The `glm` provider talks to Z.ai's API and needs a key: it's stored in the **OS keychain** (`src-tauri/src/settings.rs`, via the `keyring` crate — NOT localStorage), and `start_session` injects it as `ANTHROPIC_AUTH_TOKEN` (+ `ANTHROPIC_BASE_URL`) into the sidecar env only when `provider=glm`. The key is never returned to the webview (`get_zai_settings` reports presence only). Linux build needs `libdbus-1-dev`; runtime needs a secret-service daemon.
- **Provider adapters.** The sidecar is provider-pluggable: `core.ts` is neutral transport; the agent loop + native→`AgentMessage` mapping live in `sidecar/providers/<id>.ts`, selected by `AGENT_PROVIDER` (Rust `start_session(provider?)`, default `claude`). Both shipped providers (`claude`, `glm`) share `anthropic-base.ts` (the SDK + native binary; GLM just points it at Z.ai's Anthropic-compatible endpoint via env) — they're thin configs, NOT forks. Adding a provider = a new adapter implementing `AgentProvider` and mapping to `AgentMessage`; DON'T put provider-specific logic in `core.ts` or the UI. A runtime needing a *different* native binary also needs its own `agent.<platform>.ts` embed (GLM reuses the `claude` one).
- **Provider switching = sidecar restart, guarded by a liveness flag.** A provider is fixed for a sidecar's life, so switching model-provider (in `ModelSelector`) calls `restart_session`, not `set_model`. To stop a superseded/stopped sidecar's reader task from emitting stale events (a late `terminated`/`models`) that would corrupt the replacement under the same worktree key, each session carries an `Arc<AtomicBool>` liveness flag (`agent.rs`); the reader checks it per event and a post-loop `Arc::ptr_eq` identity-check gates cleanup. Don't reintroduce frontend stale-event guards — this is solved in Rust.
...
- **Pinned config.** systemPrompt is the `claude_code` preset + `bypassPermissions` for both providers (set in `anthropic-base.ts`). Default model is `claude-opus-4-8` (`providers/claude.ts`) or `glm-5.2` (`providers/glm.ts`). Agent cwd comes from `PROJECT_DIR` (set by Rust `start_session`), read in `core.ts` and passed as `ctx.projectDir`, defaulting to `process.cwd()`.
...
- **Persistence.** Repos, selected worktree, per-worktree model, **per-worktree provider** (`trickshot.providerByWorktree`, default `claude` — re-applied on launch/selection so a worktree reopens on its last provider), theme, **transcript**, and **agent session id** persist to `localStorage` (keys `trickshot.*`). The Z.ai key is the ONE thing NOT in localStorage — it's in the OS keychain (see the auth gotcha). Switching provider starts a FRESH agent context (the session id is dropped via `forgetWorktreeSession`); the transcript stays visible but the new provider doesn't inherit it. Chat survives restarts via TWO mechanisms that must both stay wired: the persisted transcript restores the *rendered* messages, and `start_session(worktree, resume)` resumes the SDK session by id to restore the agent's *context* — the SDK does NOT replay messages on resume, so neither alone is enough. The session id rides on every SDK message (`m.session_id`); App.svelte captures the latest. Transcripts are saved on a 600ms idle debounce (never mid-stream) and tolerate localStorage quota errors. Worktree lists are still repopulated from git (`list_worktrees`) on launch — git is the source of truth, not a persisted list.
