...
| The chat pane (the real Claude Code TUI; MULTIPLE concurrent chats per worktree, rendered as tabs or an n-up grid) | `ChatTabs.svelte` (the strip, on the shell band OUTSIDE the card — App.svelte mounts it) + `ClaudeTerminalPane.svelte` (the in-card surface: focused cell / grid) + `ClaudeTerminalCell.svelte` (one chat's terminal + its docked `ChatComposer` — the custom input row that injects via `sendToCli(…, chatId)`; the ONE composer row, also wrapped by Home's `HomeComposer`) + `stores.ts › chatSessionsByWorktree/focusedChatByWorktree/chatLayout/chatStatusByKey` + `session.ts › ensureClaudeOpen/sendToCli/submitTurnToChat/handleCliExit` + `agent.rs › latest_session_id` |
