...
- **App CSS lives in `@layer components`** (`app.css`) so Tailwind's `utilities` layer always wins for shadcn primitives. The ONLY element selectors are the deliberately-unlayered `cursor` rules at the foot of `app.css` (`input,textarea,[contenteditable] { cursor:text }` / `button:not(:disabled),a,[role=button],… { cursor:pointer }`) — they are intentionally outside `@layer components` so they DO reach `<button data-slot="button">` and restore the right cursor over the themed pointer (`themedCursor.ts`). They set only `cursor`, so the bleed is wanted. Any OTHER element selector belongs in `@layer components`, or it bleeds onto shadcn primitives.
