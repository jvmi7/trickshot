...
| Integrated terminal (PTYs per worktree: shell + claude slots) | `src-tauri/src/terminal.rs` (portable-pty commands, `launch` whitelist, `FlowGate` output flow control) + `src/lib/terminal.ts` (xterm cache + `term-event` router + `attachTerminal` + `claudeTermKey` + `syncRenderer` WebGL/DOM policy) + `TerminalPane.svelte`/`ClaudeTerminalPane.svelte`; dev-only throughput bench: `src/lib/termBench.ts` (`__termBench()` in the devtools console) |
...
- **The PTY round-trip stays thin.** `term-event` data goes straight into the cached xterm instance (`terminal.ts › handleTermEvent`) — no parsing, no store writes per chunk. The only per-chunk work besides `term.write()` is `noteCliActivity`'s O(1) tracker update and the O(1) flow-control ack batcher (`ACK_EVERY`: one `term_ack` invoke per 8 parsed chunks). Don't add per-chunk allocation, regex, or store traffic on this path.
- **The renderer is WebGL wherever the theme allows.** `terminal.ts › syncRenderer` loads `@xterm/addon-webgl` for glow-free themes and keeps GLOW themes (`--base-term-glow ≠ none`) on the DOM renderer — the glow is a text-shadow on real spans that canvas glyphs escape. Renderer choice is PER THEME, re-evaluated by `setTheme`/`attachTerminal`; don't hardcode either renderer, and don't add a third path (a `new WebglAddon()` throw marks WebGL unavailable for the session; context loss falls back per instance and retries on the next attach).
...
- **Rust relays do ZERO transform work per chunk.** PTY output and script lines are emitted as-is in the `WorktreeEvent` envelope — no per-chunk transform beyond UTF-8 boundary handling (64KiB reads, so a flood ships as few large events). The only other per-chunk work is `FlowGate`'s O(1) counter: the reader parks at 64 unacked events until the webview's `term_ack`s catch up (backpressure reaches the child via the filling PTY buffer; a stalled webview fails open after 1s). Parsing belongs to the webview consumer.
...
- **Measure before tuning** the 16ms window, the CLI idle threshold (`cliActivity.ts › CLI_IDLE_MS`), the flow-control watermarks (`terminal.rs › FLOW_HIGH/FLOW_RESUME`, `terminal.ts › ACK_EVERY`), or the script-tail cap — profile a real TUI repaint burst and a chatty dev server, not a hello-world. The dev console's `__termBench()` (`termBench.ts`) measures the webview parse+render leg in isolation; run it before/after any renderer or transport change.
