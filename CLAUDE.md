...
- **Migration tracker** (so you don't have to re-derive each component's mode). Migrate a file when you're already editing it; don't churn the others just to convert.
  - ✅ **Runes** (don't regress to legacy): `Settings`, `ModelSelector`, `LoadingState`, `PermissionModal`, `ScrollIndicator`, `HeaderIconButton`, `Message`, `Markdown`, `ToolActivity`, `ToolGroup`, and all `ui/*`.
