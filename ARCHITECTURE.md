...
| `list_listeners` | — | `Listener[]` | All listening TCP servers rooted in trickshot-spawned processes (run-script groups + PTY children; one `ps` snapshot + at most one pid-scoped `lsof`), as `{ worktree, pid, port, command }` rows. Powers the header's running-ports chips; polled coarsely by the webview, zero lsof when nothing is running |
...
| `worktree_rebase_default` | `worktreePath` | `string` | Fleet sync: fetch + `git rebase --autostash origin/<default>` (default resolved like the ahead-of-default stat). A conflicted rebase AUTO-ABORTS back to the pre-rebase state and rejects with git's message — the webview escalates to `run_git_agent` |
| `run_git_agent` | `worktreePath, prompt` | `string` | A BACKGROUND git agent: headless `claude -p` in the worktree with tools locked to `Bash(git:*)` IN RUST (the webview supplies prompts, never capabilities — the run_script posture applied to agents). No PTY, no chat session, invisible; used by Save escalation and fleet-sync conflict resolution (`src-tauri/src/generate.rs`) |
...
| `get_scripts_source` | `repoPath` | `string` | Raw text of `.trickshot/settings.json` for the in-app scripts editor (missing file → a starter template buffer; nothing is written) |
| `save_scripts_source` | `repoPath, content` | `ScriptsConfig` | Validate + write `.trickshot/settings.json` (creating `.trickshot/` on first save) and return the parsed config. Invalid JSON is rejected before touching disk |
