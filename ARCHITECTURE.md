...
| `get_volume` | — | `VolumeInfo` | The system output volume + mute state (`src-tauri/src/volume.rs`, shelling macOS `osascript` — the git/`gh` "drive the system's tool" posture, no audio crate). Rejects on non-macOS or on output devices with no software volume; the footer control hides itself on a failed probe |
| `set_volume` | `volume` | `void` | Sets the system output volume (0–100, clamped in Rust) and unmutes — matching the OS volume keys' behavior. Slider drags are throttled in the webview (one osascript spawn per ~80ms, not per pixel) |
| `set_muted` | `muted` | `void` | Mutes/unmutes the system output (the footer speaker icon's click) |
